...
public class GetStopAudits
{
    ...
    [FunctionName("GetStopAudits_v2")]
    [OpenApiOperation(operationId: "v2/GetStopAudits", tags: new[] { "name", "v2" })]
    ...
    public async Task<IActionResult> Run(
        [HttpTrigger(AuthorizationLevel.Function, "get", Route = "v2/GetStopAudits")] HttpRequest req,
        ILogger log)
    {
        ...
        string queryStringV1 = $"SELECT * FROM c WHERE Substring(c.id, 0, 12) = \"{id}\" AND (c.StopVersion = 1 OR NOT IS_DEFINED(c.StopVersion) OR IS_NULL(c.StopVersion))";
